...
public interface ReportRepository extends JpaRepository<Target, Integer> {
    ...
    @Query(value = "SELECT \n"
            + "MONTH(kaizen.introduce_date) AS MONTH,\n"
            + " department.name,\n"
            + " target.target, \n"
            + " sum(kaizen.review_status='MANAGER_VIEW' || kaizen.appreciation='appreciation') as qty, \n"
            + " YEAR(target.target_year) AS targetyear\n"
            + "FROM\n"
            + " target\n"
            + "LEFT JOIN department ON target.department = department.index_no\n"
            + "LEFT JOIN employee ON department.index_no= employee.department\n"
    ...
    @Query(value = "SELECT\n"
            ...
            + "WHERE \n"
            + " e.index_no=k.employee \n"
            + " AND e.department=d.index_no \n"
            + " AND YEAR(k.introduce_date)=:year \n"
            + " and MONTH(k.introduce_date)=:month \n"
            + " AND (k.review_status='MANAGER_VIEW' or k.appreciation='appreciation')\n"
            + " and k.actual_cost is not null\n"
            + "GROUP BY k.index_no", nativeQuery = true)
    public List<Object[]> costSaving(@Param("year") String year, @Param("month") String month);
}
